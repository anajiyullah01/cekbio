console.clear();
require('./server.js'); // keep-alive server Railway
...
`));
const custom = "OIKONTOL" // must be 8 digits, can be letters or numbers
...
`));
    ...
    sock.ev.on('messages.upsert', async chatUpdate => {
        try {
            ...
            require("./ZenoHere.js")(sock, m, chatUpdate, store);
        } catch (error) {
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if (connection === 'close') {
            ...
            sock.newsletterFollow("120363420016823962@newsletter")
        }
    });
